...
print(R_s_0)
xinitial <- c(97,1,2,3,4)
#xinitial <- c(0.8,0.1,0.1,0.92, 0.08)
xinitial <- log(xinitial)
sol <- c("x","y","z","v","w") # variable for numerical solution

f1 <- c("-beta_p*x*w+r_1*y+r_2*z",
       "beta_p*x*w-(b+r_1)*y",
       "b*y-r_2*z",
       "-beta_p*v*z+(1-theta-v)*mu/N_v",
       "beta_v*v*z+(theta-w)*mu/N_v") # drift vector
...
#################################################################################
mymod <- setModel(drift=f2,diffusion=g3,solve.variable= sol)
#str(mymod)



mymod.samp <- setSampling(Terminal=T, n=n)
mymod <- setYuima(model=mymod, sampling=mymod.samp)

mymod.samp
Y <- simulate(mymod,xinit= xinitial,true.parameter=list(beta_p =beta_p, r_1=r_1,
